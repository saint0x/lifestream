# Revenue Model

## The Fundamental Business Model

Vanta intentionally avoids relying on subscriptions.

That choice is creator-benefit focused as much as it is strategic. The platform should help creators earn from the audience value around their work without asking viewers to cross a paywall first or asking creators to pay SaaS fees before the model has created value for them.

There are three primary revenue engines:

1. Creator Advertising Marketplace
2. Platform Advertising
3. Audience Support

These systems reinforce one another.

---

## Revenue Engine 1: Creator Advertising Marketplace

This is potentially the most important component of the model.

Vanta builds an internal advertising marketplace connecting:

> Brands <-> Creators

Rather than requiring every creator to individually find sponsors, negotiate contracts, prove analytics, invoice companies, and manage campaigns, Vanta aggregates the entire process.

Vanta can approach advertisers and effectively say:

> We represent advertising inventory across a portfolio of exclusive creator media properties.

The advertiser is not simply purchasing generic impressions.

It can evaluate:

- Creator
- Series
- Episode
- Genre
- Audience
- Engagement
- Watch time
- Qualified viewers
- Audience demographics
- Historical performance

The advertiser can then purchase inventory associated with specific creators or content.

---

## Why This Can Pay Creators More

Traditional creator monetization often requires enormous scale.

A creator may generate hundreds of thousands of views and receive relatively little revenue because their content participates in a massive standardized advertising marketplace.

Vanta instead attempts to move creators closer to the economics of direct sponsorships and premium media advertising.

Rather than saying:

> You generated X views, so here is your share of generic ad revenue.

Vanta can say:

> A company paid $50,000 to sponsor this series.

The economics become based on the value of the commercial agreement, rather than purely the number of commodity impressions.

Vanta can then distribute a significant portion of that deal directly to the creator.

This potentially allows an episode with a relatively modest but highly valuable audience to generate dramatically more revenue than the same number of views on a conventional social platform.

---

## Vanta As Advertising Broker

Vanta also earns revenue for facilitating these transactions.

The platform performs:

- Creator aggregation
- Audience verification
- Analytics
- Inventory creation
- Brand discovery
- Creator discovery
- Campaign matching
- Campaign execution
- Measurement
- Payment processing

Vanta can therefore retain a platform fee, marketplace fee, or brokerage percentage from advertising deals.

The transaction becomes:

```text
Advertiser pays campaign budget
-> Creator receives majority share
-> Vanta retains marketplace fee
```

This creates revenue without requiring creators to pay upfront.

Vanta makes money when creators make money. That alignment is strategically important because the platform's self-interest should push it to help creators build better properties, attract better advertisers, and keep more of the value their audiences create.

---

## Revenue Engine 2: Vanta's Own Advertising Inventory

Vanta simultaneously owns valuable advertising inventory itself.

There are effectively two classes of inventory:

### Creator Inventory

Advertising attached directly to a creator, series, episode, livestream, or creator property.

### Platform Inventory

Advertising associated with Vanta itself.

Platform inventory could eventually include:

- Homepage placements
- Featured banners
- Discovery-page sponsorships
- Category sponsorships
- Search placements
- Featured-series placements
- Video advertising
- Interstitials
- Event sponsorships
- Live-stream advertising
- Promotional placements
- Branded discovery experiences

This is where creator acquisition becomes especially valuable.

---

## Creators Increase Platform Advertising Value

Every successful creator brings an audience.

That audience does not only view the creator's content. They enter the Vanta ecosystem.

As Vanta acquires more creators:

```text
More creators
-> More exclusive content
-> More external promotion
-> More viewers
-> More watch time
-> More platform traffic
-> More advertising inventory
-> More valuable advertising inventory
```

Vanta therefore receives an indirect economic benefit every time a creator successfully grows their Vanta property.

---

## Revenue Engine 3: Direct Audience Support

The third revenue engine is direct financial support.

Viewers can financially support creators they enjoy.

The economics are intentionally creator-friendly:

| Party | Share |
| :---- | ----: |
| Creator | 90% |
| Vanta | 10% |

For example, if a creator receives $10,000 in audience support:

| Party | Amount |
| :---- | ----: |
| Creator | $9,000 |
| Vanta | $1,000 |

This creates another monetization mechanism without requiring either party to pay a subscription.

---

## Future Optional Layer: Vanta Plus

Vanta's core platform should remain free. That principle does not prevent Vanta from eventually offering Vanta Plus as an optional subscription product for serious creators.

The distinction is important:

```text
Vanta Free = Run your channel.
Vanta Plus = Run your studio.
```

Vanta Plus should not be a paywall around publishing, distribution, or basic monetization. It should be a studio operating system for creators who need more leverage: production planning, creator automations, studio memory, audience intelligence, sponsorship operations, collaborator workflows, IP libraries, and business reporting.

The creator-benefit logic is direct. Free Vanta gives creators the environment to publish stronger work, bring audiences into a premium viewing experience, and turn qualified attention into advertiser value. Vanta Plus helps the creator create more heat around that system by improving the quality, consistency, promotion, coordination, and business operation behind the work.

In other words, Vanta Plus is not "premium Vanta." It is optional infrastructure for creators who are starting to operate like media companies.

---

## Why Vanta Can Be Completely Free

This is one of the most important consequences of the model.

Vanta does not fundamentally require:

- Viewer subscriptions
- Creator subscriptions
- Mandatory memberships
- Paywalls

The platform can remain free.

Creators pay with:

> Content + audience distribution

Viewers pay with:

> Attention

Advertisers pay with:

> Capital

Vanta monetizes the interaction between the three.

This produces a simple economic structure:

```text
Creators create attention.
Viewers provide attention.
Advertisers purchase access to attention.
Vanta operates the marketplace and infrastructure.
```

---

## Frictionless Viewing

Because Vanta does not depend upon subscriptions, content does not necessarily need to be gated behind registration.

A viewer could click a creator's Vanta link and immediately begin watching.

This is strategically important.

If an Instagram creator posts:

> Watch Episode 1 here.

the audience should ideally move:

```text
Instagram -> Link -> Video
```

rather than:

```text
Instagram -> Link -> Signup -> Email Verification -> Login -> Video
```

Removing account requirements lowers migration friction.

Vanta can still strongly encourage account creation because registered viewers produce:

- Better recommendations
- Watch histories
- Creator following
- Notifications
- Better audience analytics
- Better advertising attribution
- Better identity verification

But registration does not necessarily need to become the gate protecting the content.
