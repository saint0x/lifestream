...
pub(crate) async fn fetch_viewer_app_state(
    ...
    let (
        ...
        fetch_continue_watching_entries_limited(
            pool,
            user_id,
            Some(VIEWER_APP_STATE_CONTINUE_WATCHING_LIMIT)
        ),
    ...
    let (notifications, sessions, connected_accounts): (
        ...
        database.list_auth_sessions(
            user_id,
            current_session_id,
            Some(VIEWER_APP_STATE_SESSIONS_LIMIT)
        ),
    ...
    let profile = user_profile_details_from_bundle(
        user.clone(),
        account_bundle.profile.clone(),
        connected_accounts,
    );
