...
pub(crate) async fn fetch_creator_series(
    pool: &SqlitePool,
    creator_id: &str,
    ...
    Ok(rows
        .into_iter()
        .map(creator_series_project_from_row)
        .collect())
}

pub(crate) async fn fetch_creator_catalog_series(
    pool: &SqlitePool,
    public_only: bool,
    ...
    let query =
        creator_catalog_series_query(series_visibility_filter(public_only), "csp.id = csp.id");
    let rows = sqlx::query(&query).bind(&now).fetch_all(pool).await?;
    let mut series = Vec::with_capacity(rows.len());
    for row in rows {
        let id: String = row.get("id");
        series.push(creator_catalog_series_from_row(
            row,
            fetch_creator_catalog_seasons(pool, &id, public_only).await?,
        ));
    }
    Ok(series)
}

pub(crate) async fn fetch_creator_catalog_series_by_slug(
    pool: &SqlitePool,
    slug: &str,
    public_only: bool,
    ...
    publish_due_scheduled_upload_releases(pool, None, None).await?;
    let now = Utc::now().to_rfc3339();
    let query = creator_catalog_series_query(series_visibility_filter(public_only), "csp.slug = ?");
    ...
    let id: String = row.get("id");
    Ok(creator_catalog_series_from_row(
        row,
        fetch_creator_catalog_seasons(pool, &id, public_only).await?,
    ))
}

pub(crate) async fn fetch_creator_catalog_series_by_id(
    pool: &SqlitePool,
    id: &str,
    public_only: bool,
) -> AppResult<CreatorCatalogSeries> {
    publish_due_scheduled_upload_releases(pool, None, None).await?;
    let now = Utc::now().to_rfc3339();
    let query = creator_catalog_series_query(series_visibility_filter(public_only), "csp.id = ?");
    let row = sqlx::query(&query)
        .bind(id)
        .bind(&now)
        .fetch_optional(pool)
    ...
    Ok(creator_catalog_series_from_row(
        row,
        fetch_creator_catalog_seasons(pool, &series_id, public_only).await?,
    ))
}

pub(crate) async fn fetch_creator_series_by_id(
    pool: &SqlitePool,
    creator_id: &str,
    id: &str,
    ...
    Ok(creator_series_project_from_row(row))
}

fn creator_catalog_series_query(visibility_filter: &str, selector: &str) -> String {
    format!(
        r#"
...
fn series_visibility_filter(public_only: bool) -> &'static str {
    if public_only {
        "u.visibility = 'public'"
    } else {
...
fn creator_series_project_from_row(row: sqlx::sqlite::SqliteRow) -> CreatorSeriesProject {
    CreatorSeriesProject {
        id: row.get("id"),
        slug: row.get("slug"),
        title: row.get("title"),
...
fn creator_catalog_series_from_row(
    row: sqlx::sqlite::SqliteRow,
    seasons: Vec<CreatorCatalogSeason>,
) -> CreatorCatalogSeries {
    CreatorCatalogSeries {
        id: row.get("id"),
        slug: row.get("slug"),
        title: row.get("title"),
...
async fn fetch_creator_catalog_seasons(
    ...
    let query = format!(
        ...
        "#,
        series_visibility_filter(public_only),
    );
