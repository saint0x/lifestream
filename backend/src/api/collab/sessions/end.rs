...
pub(crate) async fn end_collaboration_session_internal_raw(
    ...
    if session.status == "ended" {
        return fetch_collaboration_session_by_id(state.db.try_sqlite_adapter()?, &session.id)
            .await;
    }

    let now = Utc::now().to_rfc3339();
    let revoked_invites = fetch_pending_collaboration_invites_for_session(
    ...
    fetch_collaboration_session_by_id(state.db.try_sqlite_adapter()?, &session.id).await
}
