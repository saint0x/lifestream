...
-> AppResult<()> {
    ...
    .bind("https://cdn.vanta.local/avatar/pending-live-summary.jpg")
    .bind("free")
    .bind(&now)
    ...
    let refreshed_invite =
        fetch_collaboration_invite_by_id(state.db.sqlite_adapter(), &pending_invite.id).await?;
    let refreshed_grant =
...
async fn metrics_report_creator_live_socket_presence_from_reconciled_state() -> AppResult<()> {
    ...
    assert!(text.contains("vanta_presence_creator_live_sockets 1"));
    assert!(text.contains("vanta_live_ingest_active_sessions"));
    assert!(text.contains("vanta_live_ingest_ready_outputs"));
    assert!(text.contains("vanta_live_ingest_artifact_attention_outputs"));
    assert!(text.contains("vanta_live_ingest_manifest_path_missing_outputs"));
    assert!(text.contains("vanta_live_ingest_archive_path_missing_outputs"));
    assert!(text.contains("vanta_live_ingest_advisory_critical_samples"));
    assert!(text.contains("vanta_live_ingest_peak_host_channel_targets"));
    assert!(text.contains("vanta_live_ingest_peak_mirror_channel_targets"));
    assert!(text.contains("vanta_live_ingest_peak_archive_targets"));
    assert!(text.contains("vanta_live_ingest_peak_active_targets"));
    assert!(text.contains("vanta_live_ingest_peak_degraded_targets"));
    assert!(text.contains("vanta_live_ingest_peak_armed_targets"));
    assert!(text.contains("vanta_live_ingest_peak_pending_source_targets"));
    Ok(())
}
...
async fn live_viewer_presence_counts_distinct_viewers_not_socket_tabs() -> AppResult<()> {
    ...
    let per_stream =
        count_active_live_viewer_sessions(state.db.sqlite_adapter(), &stream_id).await?;
    let all_active = count_all_active_live_viewer_sessions(state.db.sqlite_adapter()).await?;
...
async fn live_stream_listing_orders_by_effective_viewers_not_stale_snapshot() -> AppResult<()> {
    let (state, creator) = setup_test_state().await?;
    let other_creator = fetch_creator_profile(state.db.sqlite_adapter(), "crt-atlas").await?;
    reset_creator_live_state(state.db.sqlite_adapter(), &other_creator).await?;

    let primary_stream_id =
        insert_live_stream_for_creator(state.db.sqlite_adapter(), &creator).await?;
    let secondary_stream_id =
        insert_live_stream_for_creator(state.db.sqlite_adapter(), &other_creator).await?;
    let now = Utc::now().to_rfc3339();
    ...
    let streams = fetch_live_streams(state.db.sqlite_adapter(), None).await?;
    ...
    let refreshed = fetch_live_streams(state.db.sqlite_adapter(), None).await?;
...
async fn category_live_totals_follow_active_stream_truth_not_snapshot_columns() -> AppResult<()> {
    ...
    let refreshed_other_creator =
        fetch_creator_profile(state.db.sqlite_adapter(), &other_creator.id).await?;
    let gaming_stream_id =
        insert_live_stream_for_creator(state.db.sqlite_adapter(), &refreshed_creator).await?;
    let music_stream_id =
        insert_live_stream_for_creator(state.db.sqlite_adapter(), &refreshed_other_creator).await?;
    let now = Utc::now().to_rfc3339();
    ...
    let gaming_detail = fetch_category_by_slug(state.db.sqlite_adapter(), "gaming").await?;
...
async fn live_viewer_resume_token_cannot_be_rebound_to_another_user() -> AppResult<()> {
    ...
    let (session_token, resumed, _) = register_live_viewer_session(
...
async fn anonymous_live_viewer_resume_token_can_upgrade_to_authenticated_user() -> AppResult<()> {
    ...
    let (session_token, resumed, _) =
        register_live_viewer_session(state.db.sqlite_adapter(), &stream_id, None, None, None)
            .await?;
    assert!(!resumed);
...
async fn stale_live_viewer_disconnect_cannot_tombstone_resumed_session() -> AppResult<()> {
    ...
    let (session_token, resumed, first_lease) = register_live_viewer_session(
        state.db.sqlite_adapter(),
        &stream_id,
        Some(&identity),
        None,
    ...
    disconnect_live_viewer_session(
        state.db.sqlite_adapter(),
        &stream_id,
        &session_token,
        &first_lease,
...
async fn stale_creator_live_disconnect_cannot_tombstone_resumed_session() -> AppResult<()> {
    ...
    let (session_token, resumed, first_lease) =
        register_creator_live_socket_session(state.db.sqlite_adapter(), &creator.id, "usr-1", None)
            .await?;
    assert!(!resumed);
    tokio::time::sleep(Duration::from_millis(5)).await;
...
async fn stale_collaboration_disconnect_cannot_tombstone_resumed_session() -> AppResult<()> {
    ...
    let (session_token, resumed, first_lease) = register_collaboration_socket_session(
        state.db.sqlite_adapter(),
        &session_view,
        &identity,
        None,
