...
async fn purchase_content_access_for_id(
    ...
    ensure_creator_can_accept_paid_transactions(state.db.try_sqlite_adapter()?, &target.creator_id)
        .await?;
    ...
    .await?;
    let buyer = fetch_user(state.db.try_sqlite_adapter()?, &identity.user_id).await?;
