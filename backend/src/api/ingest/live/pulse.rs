...
pub(crate) async fn heartbeat_live_ingest(
    ...
    let session =
        validate_live_ingest_session(state.db.try_sqlite_adapter()?, &session_id, &ingest_token)
            .await?;
    ...
    let runtime_output =
        fetch_live_runtime_output_for_session(state.db.try_sqlite_adapter()?, &session_id).await?;
