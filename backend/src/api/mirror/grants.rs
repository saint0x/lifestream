...
pub(crate) async fn issue_mirror_grant_for_participant(
    ...
    .await?;
    fetch_collaboration_mirror_grant_by_id(state.db.try_sqlite_adapter()?, &grant_id).await
}
...
pub(crate) async fn redeem_collaboration_mirror_grant_internal(
    ...
    let grant =
        fetch_collaboration_mirror_grant_by_id(state.db.try_sqlite_adapter()?, grant_id).await?;
    let participant = fetch_collaboration_participant_by_id(
        state.db.try_sqlite_adapter()?,
        &grant.participant_id,
    )
    .await?;
    let session =
        fetch_collaboration_session_by_id(state.db.try_sqlite_adapter()?, &grant.session_id)
            .await?;
