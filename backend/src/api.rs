...
async fn creator_dashboard_payload(
    pool: &SqlitePool,
...
async fn fetch_creator_app_state(
    pool: &SqlitePool,
    identity: &RequestIdentity,
...
async fn list_admin_notification_deliveries(
    State(state): State<SharedState>,
    headers: HeaderMap,
...
async fn get_admin_notification_delivery(
    State(state): State<SharedState>,
    headers: HeaderMap,
...
async fn reconcile_admin_notification_delivery(
    State(state): State<SharedState>,
    headers: HeaderMap,
    Path(delivery_id): Path<String>,
...
async fn retry_admin_notification_delivery(
    State(state): State<SharedState>,
    headers: HeaderMap,
    Path(delivery_id): Path<String>,
) -> AppResult<Json<NotificationDeliveryRecord>> {
...
async fn list_admin_media_jobs(
    State(state): State<SharedState>,
    headers: HeaderMap,
...
async fn reconcile_admin_media_job(
    State(state): State<SharedState>,
    headers: HeaderMap,
    Path(job_id): Path<String>,
...
async fn retry_admin_media_job(
    State(state): State<SharedState>,
    headers: HeaderMap,
    Path(job_id): Path<String>,
) -> AppResult<Json<AdminMediaJobRecord>> {
...
async fn list_admin_live_ingest_sessions(
    State(state): State<SharedState>,
    headers: HeaderMap,
...
async fn get_admin_live_ingest_session(
    State(state): State<SharedState>,
    headers: HeaderMap,
...
async fn reconcile_admin_live_ingest_session(
    State(state): State<SharedState>,
    headers: HeaderMap,
    Path(session_id): Path<String>,
...
async fn terminate_admin_live_ingest_session(
    State(state): State<SharedState>,
    headers: HeaderMap,
    Path(session_id): Path<String>,
...
async fn list_admin_playback_sessions(
    State(state): State<SharedState>,
    headers: HeaderMap,
...
async fn get_admin_playback_session(
    State(state): State<SharedState>,
    headers: HeaderMap,
    Path(session_id): Path<String>,
...
async fn reconcile_admin_playback_session(
    State(state): State<SharedState>,
    headers: HeaderMap,
    Path(session_id): Path<String>,
...
async fn revoke_admin_playback_session(
    State(state): State<SharedState>,
    headers: HeaderMap,
    Path(session_id): Path<String>,
) -> AppResult<Json<AdminPlaybackSessionRecord>> {
...
async fn get_creator_live_ingest_session(
    State(state): State<SharedState>,
    headers: HeaderMap,
...
async fn get_creator_live_ingest_session_by_id(
    State(state): State<SharedState>,
    headers: HeaderMap,
    Path(session_id): Path<String>,
) -> AppResult<Json<AdminLiveIngestSessionRecord>> {
...
async fn list_creator_live_ingest_events(
    State(state): State<SharedState>,
    headers: HeaderMap,
    Path(session_id): Path<String>,
...
async fn reconcile_creator_live_ingest_session(
    State(state): State<SharedState>,
    headers: HeaderMap,
    Path(session_id): Path<String>,
) -> AppResult<Json<LiveIngestReconciliationReport>> {
...
async fn terminate_creator_live_ingest(
    State(state): State<SharedState>,
    headers: HeaderMap,
    Path(session_id): Path<String>,
    Json(input): Json<TerminateLiveIngestRequest>,
...
async fn connect_live_ingest(
    ...
    .bind(crate::auth::hash_token(input.stream_key.trim()))
    .bind(crate::auth::hash_token(&ingest_token))
...
async fn heartbeat_live_ingest(
    State(state): State<SharedState>,
    Path(session_id): Path<String>,
    headers: HeaderMap,
...
async fn disconnect_live_ingest(
    State(state): State<SharedState>,
    Path(session_id): Path<String>,
    headers: HeaderMap,
) -> AppResult<Json<LiveIngestSession>> {
...
async fn update_upload_lifecycle(
    State(state): State<SharedState>,
    headers: HeaderMap,
    Path(id): Path<String>,
...
async fn get_upload_ingest_session(
    State(state): State<SharedState>,
    headers: HeaderMap,
    Path(id): Path<String>,
...
async fn start_upload_ingest_session(
    State(state): State<SharedState>,
    headers: HeaderMap,
    Path(id): Path<String>,
...
async fn complete_upload_ingest(
    State(state): State<SharedState>,
    headers: HeaderMap,
    Path(id): Path<String>,
) -> AppResult<Json<UploadJob>> {
...
async fn retry_upload_job_processing(
    State(state): State<SharedState>,
    headers: HeaderMap,
    Path(id): Path<String>,
) -> AppResult<Json<UploadJob>> {
...
async fn get_media_asset_for_upload_job(
    State(state): State<SharedState>,
    headers: HeaderMap,
    Path(id): Path<String>,
...
async fn create_upload_playback_session(
    State(state): State<SharedState>,
    headers: HeaderMap,
...
async fn create_content_playback_session(
    State(state): State<SharedState>,
    headers: HeaderMap,
...
async fn create_live_playback_session(
    State(state): State<SharedState>,
    headers: HeaderMap,
...
async fn purchase_upload_access(
    State(state): State<SharedState>,
    headers: HeaderMap,
    Path(upload_id): Path<String>,
...
async fn purchase_content_access(
    State(state): State<SharedState>,
    headers: HeaderMap,
    Path(content_id): Path<String>,
) -> AppResult<Json<ContentPurchase>> {
...
async fn refresh_playback_session(
    State(state): State<SharedState>,
    Path(session_id): Path<String>,
    Query(query): Query<PlaybackAccessQuery>,
) -> AppResult<Json<PlaybackGrant>> {
    ...
    .bind(crate::auth::hash_token(&refreshed_token))
...
async fn get_playback_manifest(
    State(state): State<SharedState>,
    Path(session_id): Path<String>,
    Query(query): Query<PlaybackAccessQuery>,
) -> AppResult<Response> {
...
fn rewrite_hls_manifest_reference(
...
fn normalize_relative_storage_path(path: &FsPath) -> PathBuf {
...
fn rewrite_hls_manifest_media_uri_line(
    line: &str,
    manifest_dir: &FsPath,
    playback_token: &str,
) -> String {
...
async fn mark_my_notification_read(
    State(state): State<SharedState>,
    headers: HeaderMap,
...
enum CollaborationSocketCommand {
    Heartbeat,
    RevokeInvite {
        invite_id: String,
    },
...
struct CollaborationSocketCommandOutcome {
...
async fn execute_collaboration_socket_command(
    state: &SharedState,
    session_id: &str,
    identity: &RequestIdentity,
...
async fn fetch_current_collaboration_socket_session_view(
    state: &SharedState,
    session_id: &str,
    identity: &RequestIdentity,
...
async fn reconcile_collaboration_session_expiry_for_read(
    state: &SharedState,
    session_id: &str,
) -> AppResult<()> {
...
async fn reconcile_collaboration_expiry_for_host_read(
    state: &SharedState,
    creator_id: &str,
) -> AppResult<()> {
    let now = Utc::now().to_rfc3339();
...
async fn reconcile_collaboration_expiry_for_participant_read(
    state: &SharedState,
    user_id: &str,
) -> AppResult<()> {
    let now = Utc::now().to_rfc3339();
...
fn auth_session_channel_id(session_id: &str) -> String {
...
async fn fetch_series_by_genre(pool: &SqlitePool, genre: &str) -> AppResult<Vec<Series>> {
    let rows = sqlx::query(
        r#"
...
async fn fetch_episode_by_id(pool: &SqlitePool, episode_id: &str) -> AppResult<Episode> {
    let row = sqlx::query(
        r#"
...
async fn fetch_films_by_genre(pool: &SqlitePool, genre: &str) -> AppResult<Vec<Film>> {
    let rows = sqlx::query(
        r#"
        SELECT id, slug, title, tagline, synopsis, year, rating, genres_json, images_json,
...
async fn fetch_streamers(pool: &SqlitePool) -> AppResult<Vec<Streamer>> {
    let rows = sqlx::query(
...
async fn fetch_streamer_by_id(pool: &SqlitePool, id: &str) -> AppResult<Streamer> {
    let row = sqlx::query(
...
async fn validate_watchlist_content(pool: &SqlitePool, content_id: &str) -> AppResult<()> {
...
async fn fetch_streamer_by_handle(pool: &SqlitePool, handle: &str) -> AppResult<Streamer> {
    let row = sqlx::query(
...
async fn fetch_live_streams_by_category(
    pool: &SqlitePool,
    category: &str,
) -> AppResult<Vec<LiveStream>> {
...
fn sort_live_streams(streams: &mut [LiveStream], sort: &str) {
    match sort {
...
async fn fetch_live_stream_by_slug(pool: &SqlitePool, slug: &str) -> AppResult<LiveStream> {
...
async fn fetch_live_stream_by_id(pool: &SqlitePool, id: &str) -> AppResult<LiveStream> {
    let fresh_cutoff = stale_live_ingest_cutoff();
    let row = sqlx::query(
        r#"
        SELECT
...
async fn fetch_live_stream_owner_creator_id(
    pool: &SqlitePool,
    stream_id: &str,
) -> AppResult<String> {
    let fresh_cutoff = stale_live_ingest_cutoff();
...
async fn authorize_live_stream_owner(
    pool: &SqlitePool,
    stream_id: &str,
    identity: &RequestIdentity,
) -> AppResult<String> {
...
async fn authorize_live_stream_moderation(
    pool: &SqlitePool,
    stream_id: &str,
    identity: &RequestIdentity,
) -> AppResult<String> {
    ...
    let row = sqlx::query("SELECT 1 FROM creator_moderators WHERE creator_id = ? AND user_id = ?")
        .bind(&creator_id)
        .bind(&identity.user_id)
        .fetch_optional(pool)
        .await?;
...
async fn can_bypass_live_chat_restrictions(
    pool: &SqlitePool,
    creator_id: &str,
    identity: &RequestIdentity,
) -> AppResult<bool> {
...
async fn validate_live_moderation_subject(
    pool: &SqlitePool,
    stream_id: &str,
    creator_id: &str,
    identity: &RequestIdentity,
    ...
    if identity.user_id == subject_user_id {
        return Err(AppError::BadRequest(
            "moderation actions cannot target the acting user".to_string(),
        ));
    }

    let owner = fetch_creator_profile(pool, creator_id).await?;
    if owner.user_id == subject_user_id {
        return Err(AppError::BadRequest(
...
async fn fetch_creator_moderators(
    pool: &SqlitePool,
    creator_id: &str,
...
async fn fetch_creator_moderator(
    pool: &SqlitePool,
    creator_id: &str,
    user_id: &str,
...
async fn fetch_live_moderation_actions(
    pool: &SqlitePool,
    stream_id: &str,
    creator_id: &str,
...
async fn fetch_live_moderation_action_by_id(
    pool: &SqlitePool,
    action_id: &str,
...
async fn fetch_live_moderation_action_by_id_raw(
    pool: &SqlitePool,
    action_id: &str,
) -> AppResult<LiveModerationAction> {
    let row = sqlx::query(
...
async fn fetch_active_live_moderation_action(
    pool: &SqlitePool,
    stream_id: &str,
    subject_user_id: &str,
...
async fn fetch_live_stream_reports(
    pool: &SqlitePool,
    stream_id: &str,
...
async fn fetch_live_stream_report_by_id(
    pool: &SqlitePool,
    report_id: &str,
...
async fn fetch_moderation_audit_log(
    pool: &SqlitePool,
    creator_id: &str,
...
async fn write_moderation_audit_entry(
    pool: &SqlitePool,
    creator_id: &str,
    stream_id: Option<&str>,
    actor_user_id: &str,
...
fn live_moderation_action_from_row(row: sqlx::sqlite::SqliteRow) -> LiveModerationAction {
...
fn creator_enforcement_action_from_row(row: sqlx::sqlite::SqliteRow) -> CreatorEnforcementAction {
...
fn notification_delivery_record_from_row(
    row: sqlx::sqlite::SqliteRow,
...
fn live_stream_report_from_row(row: sqlx::sqlite::SqliteRow) -> LiveStreamReportRecord {
...
fn validate_creator_moderator_role(role: &str) -> AppResult<()> {
    match role {
        "mod" | "admin" => Ok(()),
        _ => Err(AppError::BadRequest(
...
fn validate_slow_mode_seconds(seconds: i64) -> AppResult<()> {
...
fn validate_auto_mod_level(level: &str) -> AppResult<()> {
    match level {
...
fn validate_live_moderation_action_type(action_type: &str) -> AppResult<()> {
    match action_type {
...
fn validate_live_report_status(status: &str) -> AppResult<()> {
    match status {
...
fn validate_creator_enforcement_scope(scope: &str) -> AppResult<()> {
    match scope.trim() {
...
fn validate_upload_visibility(visibility: &str) -> AppResult<()> {
    match visibility {
...
fn validate_upload_job_kind(kind: &str) -> AppResult<()> {
    match kind {
...
fn validate_upload_job_source_type(source_type: &str) -> AppResult<()> {
    match source_type {
...
fn derive_upload_lifecycle_status(
    current_status: &str,
    visibility: &str,
...
fn validate_bulk_upload_action(upload: &Upload, action: &str) -> AppResult<()> {
    match action {
        "archive" => {
...
fn summarize_creator_content(uploads: &[Upload], filtered_count: i64) -> CreatorContentSummary {
...
async fn fetch_category_by_slug(pool: &SqlitePool, slug: &str) -> AppResult<Category> {
    let row = sqlx::query(
...
async fn fetch_user_library(pool: &SqlitePool, user_id: &str) -> AppResult<UserLibrary> {
...
async fn fetch_watchlist_response(
    pool: &SqlitePool,
    user_id: &str,
...
async fn fetch_viewer_app_state(
    pool: &SqlitePool,
    user_id: &str,
...
async fn fetch_followed_streamer_ids(pool: &SqlitePool, user_id: &str) -> AppResult<Vec<String>> {
    Ok(
        sqlx::query(
...
async fn fetch_creator_id_for_user(pool: &SqlitePool, user_id: &str) -> AppResult<Option<String>> {
...
async fn fetch_user_profile_details(
    pool: &SqlitePool,
    user_id: &str,
...
async fn fetch_user_settings_bundle(
    pool: &SqlitePool,
    user_id: &str,
...
async fn fetch_billing_plan(pool: &SqlitePool, user_id: &str) -> AppResult<BillingPlan> {
    let row = sqlx::query(
        r#"
...
async fn fetch_creator_profile(pool: &SqlitePool, creator_id: &str) -> AppResult<CreatorProfile> {
    let row = sqlx::query(
        r#"
...
async fn fetch_creator_profile_by_stream_key(
    pool: &SqlitePool,
    stream_key: &str,
...
async fn fetch_creator_operational_state(
    pool: &SqlitePool,
...
async fn fetch_creator_enforcement_state(
    pool: &SqlitePool,
    profile: &CreatorProfile,
...
async fn fetch_creator_enforcement_actions(
    pool: &SqlitePool,
    creator_id: &str,
...
async fn fetch_active_creator_enforcement_actions(
    pool: &SqlitePool,
    creator_id: &str,
) -> AppResult<Vec<CreatorEnforcementAction>> {
    let now = Utc::now().to_rfc3339();
...
async fn fetch_creator_enforcement_action_by_id(
    pool: &SqlitePool,
    action_id: &str,
...
async fn fetch_creator_enforcement_action_by_id_raw(
    pool: &SqlitePool,
    action_id: &str,
) -> AppResult<CreatorEnforcementAction> {
    let row = sqlx::query(
...
async fn ensure_creator_live_settings_row(pool: &SqlitePool, creator_id: &str) -> AppResult<()> {
    sqlx::query(
        r#"
...
async fn fetch_creator_live_settings(
    pool: &SqlitePool,
    creator_id: &str,
...
async fn fetch_creator_live_health(
    pool: &SqlitePool,
    creator_id: &str,
...
async fn fetch_creator_subscriber_tiers(
    pool: &SqlitePool,
    creator_id: &str,
...
async fn fetch_creator_subscriber_tier_by_id(
    pool: &SqlitePool,
    creator_id: &str,
    tier_id: &str,
...
async fn next_creator_subscriber_tier_rank(pool: &SqlitePool, creator_id: &str) -> AppResult<i64> {
    let row = sqlx::query(
...
async fn normalize_creator_subscriber_tier_ranks(
    pool: &SqlitePool,
    creator_id: &str,
) -> AppResult<()> {
    let rows = sqlx::query(
...
fn validate_creator_subscriber_tier_input(
    tier_name: &str,
    rank: Option<i64>,
    monthly_price: f64,
    accent_color: &str,
...
async fn fetch_creator_membership(
    pool: &SqlitePool,
    user_id: &str,
    creator_id: &str,
...
async fn fetch_content_purchase_by_id(
    pool: &SqlitePool,
    purchase_id: &str,
...
async fn purchase_belongs_to_user(
    pool: &SqlitePool,
    user_id: &str,
    purchase_id: &str,
) -> AppResult<bool> {
...
async fn fetch_current_content_purchase(
    pool: &SqlitePool,
    user_id: &str,
    upload_id: &str,
...
async fn fetch_user_entitlements(pool: &SqlitePool, user_id: &str) -> AppResult<UserEntitlements> {
...
async fn reconcile_single_membership_entitlement(
    state: SharedState,
    user_id: &str,
    creator_id: &str,
) -> AppResult<CreatorMembershipReconciliationReport> {
    let before = fetch_creator_membership(&state.pool, user_id, creator_id).await?;
    let now = Utc::now().to_rfc3339();
    ...
    let membership = fetch_creator_membership(&state.pool, user_id, creator_id).await?;
...
async fn reconcile_single_purchase_entitlement(
    state: SharedState,
    user_id: &str,
    purchase_id: &str,
) -> AppResult<ContentPurchaseReconciliationReport> {
    let before = fetch_content_purchase_by_id(&state.pool, purchase_id).await?;
    let now = Utc::now().to_rfc3339();
    let mut actions = Vec::new();
...
async fn fetch_creator_catalog_series(
    pool: &SqlitePool,
    public_only: bool,
...
async fn fetch_creator_catalog_series_by_slug(
    pool: &SqlitePool,
    slug: &str,
    public_only: bool,
...
async fn fetch_creator_catalog_series_by_id(
    pool: &SqlitePool,
    id: &str,
    public_only: bool,
) -> AppResult<CreatorCatalogSeries> {
...
async fn fetch_creator_catalog_films(
    pool: &SqlitePool,
    public_only: bool,
...
async fn fetch_creator_catalog_film_by_slug(
    pool: &SqlitePool,
    slug: &str,
    public_only: bool,
...
async fn fetch_creator_catalog_film_by_id(
    pool: &SqlitePool,
    id: &str,
    public_only: bool,
) -> AppResult<CreatorCatalogFilm> {
...
async fn fetch_creator_series_by_id(
    pool: &SqlitePool,
    creator_id: &str,
    id: &str,
...
async fn fetch_broadcasts(pool: &SqlitePool, creator_id: &str) -> AppResult<Vec<Broadcast>> {
    let rows = sqlx::query(
...
async fn fetch_broadcast_by_id(
    pool: &SqlitePool,
    creator_id: &str,
    id: &str,
) -> AppResult<Broadcast> {
...
async fn resolve_collaboration_broadcast(
    pool: &SqlitePool,
    creator_id: &str,
...
async fn fetch_collaboration_sessions_for_host(
    pool: &SqlitePool,
    creator_id: &str,
...
async fn fetch_collaboration_sessions_for_participant(
    pool: &SqlitePool,
    user_id: &str,
...
async fn fetch_collaboration_session_by_id(
    pool: &SqlitePool,
    session_id: &str,
...
async fn fetch_active_collaboration_session_for_broadcast(
    pool: &SqlitePool,
    broadcast_id: &str,
...
async fn end_collaboration_session_internal_raw(
    state: &SharedState,
...
async fn end_collaboration_session_internal(
    state: &SharedState,
    session: &CollaborationSession,
    actor_user_id: Option<String>,
    payload: Value,
...
async fn fetch_collaboration_session_for_host(
    pool: &SqlitePool,
    creator_id: &str,
    session_id: &str,
) -> AppResult<CollaborationSession> {
...
async fn fetch_collaboration_session_for_participant(
    pool: &SqlitePool,
    user_id: &str,
    session_id: &str,
) -> AppResult<CollaborationSessionView> {
...
fn validate_collaboration_participant_access(
...
fn collaboration_session_view_for_host(
    session: CollaborationSession,
...
async fn fetch_collaboration_invites_for_session(
    pool: &SqlitePool,
    session_id: &str,
...
async fn fetch_pending_collaboration_invites_for_session(
    pool: &SqlitePool,
    session_id: &str,
) -> AppResult<Vec<CollaborationInvite>> {
...
async fn fetch_collaboration_participants_for_session(
    pool: &SqlitePool,
    session_id: &str,
...
async fn fetch_collaboration_invite_by_id(
    pool: &SqlitePool,
    invite_id: &str,
...
async fn fetch_collaboration_participant_by_id(
    pool: &SqlitePool,
    participant_id: &str,
...
async fn fetch_collaboration_participant_for_user(
    pool: &SqlitePool,
    session_id: &str,
    user_id: &str,
) -> AppResult<CollaborationParticipant> {
...
async fn fetch_collaboration_invites_for_user(
    pool: &SqlitePool,
    user_id: &str,
) -> AppResult<Vec<CollaborationInvite>> {
    let rows = sqlx::query(
...
async fn fetch_collaboration_host_summary(
    pool: &SqlitePool,
    creator_id: &str,
...
async fn has_pending_collaboration_invite_for_user(
    pool: &SqlitePool,
    session_id: &str,
    user_id: &str,
) -> AppResult<bool> {
...
async fn fetch_collaboration_events(
    pool: &SqlitePool,
    session_id: &str,
    after_seq: i64,
    limit: i64,
...
async fn load_collaboration_socket_event_bootstrap(
    pool: &SqlitePool,
    session_id: &str,
    after_seq: i64,
...
fn collaboration_event_is_visible_to_session(
    session: &CollaborationSessionView,
...
fn filter_visible_collaboration_events_for_session(
    session: &CollaborationSessionView,
...
async fn fetch_collaboration_mirror_grants_for_participant(
    pool: &SqlitePool,
    participant_id: &str,
...
async fn fetch_collaboration_mirror_grants_for_session(
    pool: &SqlitePool,
    session_id: &str,
) -> AppResult<Vec<CollaborationMirrorGrant>> {
    let rows = sqlx::query(
...
async fn fetch_collaboration_mirror_pickups_for_participant(
    pool: &SqlitePool,
    participant_id: &str,
...
async fn fetch_collaboration_mirror_pickups_for_session(
    pool: &SqlitePool,
    session_id: &str,
) -> AppResult<Vec<CollaborationMirrorPickup>> {
    let rows = sqlx::query(
...
async fn fetch_visible_collaboration_mirror_pickups_for_session_view(
    pool: &SqlitePool,
    session: &CollaborationSessionView,
) -> AppResult<Vec<CollaborationMirrorPickup>> {
    if session.participant.role == "host" {
...
async fn fetch_visible_collaboration_mirror_grants_for_session_view(
    pool: &SqlitePool,
    session: &CollaborationSessionView,
) -> AppResult<Vec<CollaborationMirrorGrant>> {
    if session.participant.role == "host" {
...
async fn fetch_collaboration_socket_presence_by_id_raw(
    pool: &SqlitePool,
    session_id: &str,
    socket_id: &str,
...
fn build_collaboration_runtime_topology(
    session: &CollaborationSessionView,
...
async fn build_collaboration_runtime_response_for_participant(
    pool: &SqlitePool,
    session: CollaborationSessionView,
...
async fn build_collaboration_runtime_response_for_host(
    pool: &SqlitePool,
    session: CollaborationSession,
) -> AppResult<CollaborationRuntimeResponse> {
    let host = fetch_collaboration_host_summary(pool, &session.host_creator_id).await?;
...
async fn build_creator_collaboration_control_response_for_host(
    pool: &SqlitePool,
    session: CollaborationSession,
...
async fn fetch_creator_live_collaboration_summary(
    pool: &SqlitePool,
    creator_id: &str,
...
async fn publish_collaboration_topology(state: &SharedState, session_id: &str) -> AppResult<()> {
...
async fn publish_collaboration_presence(state: &SharedState, session_id: &str) -> AppResult<()> {
    let connected_participants =
...
async fn expire_pending_collaboration_invites_for_session(
    state: &SharedState,
    session_id: &str,
    now: &str,
...
async fn expire_collaboration_mirror_grants_for_session(
    state: &SharedState,
    session_id: &str,
    now: &str,
) -> AppResult<Vec<CollaborationReconciliationAction>> {
...
async fn disconnect_stale_collaboration_socket_sessions_for_session(
    state: &SharedState,
    session_id: &str,
    now: &str,
    cutoff: &str,
...
async fn reconcile_single_collaboration_socket_session(
    state: SharedState,
    session_id: &str,
    socket_id: &str,
...
async fn reconcile_single_collaboration_session(
    state: SharedState,
    session_id: &str,
...
async fn fetch_collaboration_mirror_grant_by_id(
    pool: &SqlitePool,
    grant_id: &str,
...
async fn fetch_revocable_collaboration_mirror_grants_for_participant(
    pool: &SqlitePool,
    participant_id: &str,
) -> AppResult<Vec<CollaborationMirrorGrant>> {
    let rows = sqlx::query(
...
async fn fetch_revocable_collaboration_mirror_grants_for_session(
    pool: &SqlitePool,
    session_id: &str,
) -> AppResult<Vec<CollaborationMirrorGrant>> {
    let rows = sqlx::query(
...
async fn fetch_active_collaboration_mirror_pickup_for_grant(
    pool: &SqlitePool,
    grant_id: &str,
...
async fn fetch_active_collaboration_mirror_pickups_for_grants(
    pool: &SqlitePool,
    grants: &[CollaborationMirrorGrant],
) -> AppResult<Vec<CollaborationMirrorPickup>> {
...
async fn deactivate_collaboration_mirror_pickups_for_grants(
    state: &SharedState,
    grants: &[CollaborationMirrorGrant],
    terminal_state: &str,
    ended_at: &str,
...
async fn sync_active_collaboration_mirror_pickups_for_session(
    pool: &SqlitePool,
    session_id: &str,
) -> AppResult<()> {
...
async fn sync_active_collaboration_mirror_pickups_for_session_and_publish(
    state: &SharedState,
    session_id: &str,
) -> AppResult<()> {
...
async fn publish_collaboration_mirror_grant_revoked_events(
    state: &SharedState,
    session_id: &str,
    actor_user_id: Option<String>,
    grants: &[CollaborationMirrorGrant],
...
async fn publish_collaboration_mirror_grant_revoked_events_raw(
    state: &SharedState,
    session_id: &str,
    actor_user_id: Option<String>,
    grants: &[CollaborationMirrorGrant],
...
async fn publish_collaboration_invite_revoked_events(
    state: &SharedState,
    session_id: &str,
    actor_user_id: Option<String>,
...
async fn publish_collaboration_invite_revoked_events_raw(
    state: &SharedState,
    session_id: &str,
    actor_user_id: Option<String>,
    invites: &[CollaborationInvite],
...
async fn revoke_collaboration_mirror_grants_for_participant(
    state: &SharedState,
    session_id: &str,
    participant_id: &str,
    actor_user_id: Option<String>,
...
async fn revoke_collaboration_mirror_grants_for_session(
    state: &SharedState,
    session_id: &str,
    actor_user_id: Option<String>,
    revoked_at: &str,
...
async fn revoke_collaboration_mirror_grants_for_session_raw(
    state: &SharedState,
    session_id: &str,
    actor_user_id: Option<String>,
    revoked_at: &str,
...
async fn publish_collaboration_event_raw(
    state: &SharedState,
    session_id: &str,
    actor_user_id: Option<String>,
    participant_id: Option<String>,
...
async fn publish_collaboration_reconciliation_event(
    state: &SharedState,
    session_id: &str,
    actor_user_id: Option<String>,
    participant_id: Option<String>,
...
async fn redeem_collaboration_mirror_grant_internal(
    state: &SharedState,
    identity: &RequestIdentity,
    grant_id: &str,
) -> AppResult<CollaborationMirrorGrant> {
    ...
    .await
    {
        Ok(pickup) => pickup,
        Err(error) => {
            // Mirror redemption is only valid if the pickup materializes too.
            sqlx::query(
...
fn collaboration_channel_id(session_id: &str) -> String {
    format!("collab:{session_id}")
}

fn creator_live_channel_id(creator_id: &str) -> String {
    format!("creator-live:{creator_id}")
}

fn stream_channel_id(stream_id: &str) -> String {
    format!("stream:{stream_id}")
}

fn playback_content_session_api_url(content_id: &str) -> String {
    format!("/api/v1/playback/content/{content_id}/session")
}

fn playback_live_session_api_url(stream_id: &str) -> String {
    format!("/api/v1/playback/live/{stream_id}/session")
}

async fn fetch_creator_live_control_response(
    pool: &SqlitePool,
    creator_id: &str,
...
async fn fetch_creator_live_runtime_response(
    pool: &SqlitePool,
    creator_id: &str,
...
async fn fetch_authoritative_creator_live_control_response(
    state: &SharedState,
    creator_id: &str,
) -> AppResult<CreatorLiveControlResponse> {
...
async fn fetch_authoritative_creator_live_runtime_response(
    state: &SharedState,
    creator_id: &str,
) -> AppResult<CreatorLiveRuntimeResponse> {
    reconcile_collaboration_expiry_for_host_read(state, creator_id).await?;
...
async fn publish_authoritative_creator_live_state(
    state: &SharedState,
    creator_id: &str,
) -> AppResult<()> {
    let event = WsEvent::CreatorLiveState {
...
async fn publish_creator_live_state(state: &SharedState, creator_id: &str) -> AppResult<()> {
    publish_authoritative_creator_live_state(state, creator_id).await
}

async fn fetch_creator_live_socket_presence_by_id_raw(
    pool: &SqlitePool,
    creator_id: &str,
    socket_id: &str,
...
async fn fetch_active_live_ingest_session(
    pool: &SqlitePool,
    creator_id: &str,
...
async fn fetch_active_live_ingest_session_unreconciled(
    pool: &SqlitePool,
    creator_id: &str,
) -> AppResult<Option<LiveIngestSession>> {
    let row = sqlx::query(
...
async fn count_live_ingest_sessions_for_broadcast(
    pool: &SqlitePool,
    creator_id: &str,
    broadcast_id: &str,
) -> AppResult<i64> {
...
async fn fetch_live_ingest_session_by_id(
    pool: &SqlitePool,
    creator_id: &str,
    session_id: &str,
...
async fn fetch_live_ingest_session_by_id_unreconciled(
    pool: &SqlitePool,
    creator_id: &str,
    session_id: &str,
) -> AppResult<LiveIngestSession> {
...
async fn fetch_recent_live_ingest_sessions(
    pool: &SqlitePool,
    creator_id: &str,
    limit: i64,
...
async fn fetch_live_ingest_session_by_id_global(
    pool: &SqlitePool,
    session_id: &str,
) -> AppResult<LiveIngestSession> {
...
async fn fetch_live_ingest_session_by_id_global_unreconciled(
    pool: &SqlitePool,
    session_id: &str,
) -> AppResult<LiveIngestSession> {
    let row = sqlx::query(
...
async fn fetch_live_ingest_events_for_session(
    pool: &SqlitePool,
    session_id: &str,
    limit: i64,
...
async fn fetch_admin_live_ingest_sessions(
    pool: &SqlitePool,
...
async fn fetch_admin_live_ingest_session_record(
    pool: &SqlitePool,
    session_id: &str,
...
async fn fetch_creator_live_ingest_session_record(
    pool: &SqlitePool,
    creator_id: &str,
    session_id: &str,
) -> AppResult<AdminLiveIngestSessionRecord> {
...
async fn write_live_ingest_event(
    pool: &SqlitePool,
    session_id: &str,
    creator_id: &str,
    broadcast_id: &str,
...
async fn fetch_live_ingest_events_for_creator(
    pool: &SqlitePool,
    creator_id: &str,
    limit: i64,
) -> AppResult<Vec<LiveIngestEvent>> {
...
async fn fetch_terminalizable_live_ingest_sessions_for_broadcast(
    pool: &SqlitePool,
    creator_id: &str,
    broadcast_id: &str,
) -> AppResult<Vec<LiveIngestSession>> {
...
async fn build_creator_live_snapshot(
    pool: &SqlitePool,
    creator_id: &str,
...
fn contract_live_status(status: &str) -> String {
    match status {
...
fn contract_broadcast_status(status: &str) -> String {
    match status {
...
fn contract_creator_profile(mut profile: CreatorProfile) -> CreatorProfile {
    profile.live_status = contract_live_status(&profile.live_status);
    profile
}

fn contract_broadcast(mut broadcast: Broadcast) -> Broadcast {
    broadcast.status = contract_broadcast_status(&broadcast.status);
    broadcast
}

fn contract_broadcasts(broadcasts: Vec<Broadcast>) -> Vec<Broadcast> {
    broadcasts.into_iter().map(contract_broadcast).collect()
}

async fn normalize_creator_live_profile(
    pool: &SqlitePool,
    creator_id: &str,
...
async fn validate_live_ingest_session(
    pool: &SqlitePool,
    session_id: &str,
    ingest_token: &str,
) -> AppResult<LiveIngestSession> {
...
async fn transition_broadcast_to_live(
    pool: &SqlitePool,
...
async fn enqueue_creator_broadcast_ended_notification(
    pool: &SqlitePool,
    creator: &CreatorProfile,
    broadcast: &Broadcast,
    terminal_status: &str,
...
async fn reset_creator_live_operational_metrics(
    pool: &SqlitePool,
    creator_id: &str,
) -> AppResult<()> {
    sqlx::query(
...
async fn ensure_live_stream_row(
    pool: &SqlitePool,
    creator: &CreatorProfile,
    broadcast: &Broadcast,
    viewers: i64,
...
async fn close_live_ingest_session(
    state: &SharedState,
...
async fn mark_live_ingest_session_stale(
    state: &SharedState,
    session: &LiveIngestSession,
) -> AppResult<()> {
...
async fn mark_live_ingest_session_stale_in_db(
    pool: &SqlitePool,
    session: &LiveIngestSession,
) -> AppResult<()> {
    let creator = fetch_creator_profile(pool, &session.creator_id).await?;
...
async fn reconcile_stale_live_ingest_sessions(state: SharedState) -> AppResult<()> {
...
async fn reconcile_stale_live_ingest_sessions_for_read(
    pool: &SqlitePool,
    creator_filter: Option<&str>,
...
async fn reconcile_single_live_ingest_session(
    state: SharedState,
    session_id: &str,
...
async fn reconcile_expired_collaboration_invites(state: SharedState) -> AppResult<()> {
    let now = Utc::now().to_rfc3339();
    let rows = sqlx::query(
        r#"
        SELECT DISTINCT session_id
...
async fn reconcile_expired_collaboration_mirror_grants(state: SharedState) -> AppResult<()> {
    let now = Utc::now().to_rfc3339();
    let rows = sqlx::query(
        r#"
        SELECT DISTINCT session_id
...
async fn reconcile_expired_user_entitlements(state: SharedState) -> AppResult<()> {
    reconcile_expired_user_entitlements_for_read(&state.pool, None).await
}

async fn reconcile_expired_user_entitlements_for_read(
    pool: &SqlitePool,
...
async fn reconcile_expired_live_moderation_actions(state: SharedState) -> AppResult<()> {
    let now = Utc::now().to_rfc3339();
    let rows = sqlx::query(
        r#"
        SELECT id, stream_id, creator_id, subject_user_id, actor_user_id, action_type, reason, state,
...
async fn reconcile_expired_live_moderation_actions_for_read(
    pool: &SqlitePool,
...
async fn reconcile_single_live_moderation_action(
    state: SharedState,
    action_id: &str,
...
async fn reconcile_expired_creator_enforcement_actions(state: SharedState) -> AppResult<()> {
    reconcile_expired_creator_enforcement_actions_for_read(&state.pool, None, None).await
}

async fn reconcile_expired_creator_enforcement_actions_for_read(
    pool: &SqlitePool,
    creator_filter: Option<&str>,
    action_filter: Option<&str>,
) -> AppResult<()> {
...
async fn reconcile_notification_deliveries(state: SharedState) -> AppResult<()> {
    let now = Utc::now().to_rfc3339();
    let rows = sqlx::query(
        r#"
        SELECT id
...
async fn reconcile_single_creator_enforcement_action(
    state: SharedState,
    action_id: &str,
...
async fn reconcile_stale_media_processing_jobs(state: SharedState) -> AppResult<()> {
...
async fn reconcile_stale_media_processing_jobs_for_read(
    pool: &SqlitePool,
    creator_filter: Option<&str>,
...
async fn reconcile_single_media_job(
    state: SharedState,
    job_id: &str,
...
async fn reconcile_scheduled_upload_releases(state: SharedState) -> AppResult<()> {
    publish_due_scheduled_upload_releases(&state.pool, None, None).await
}

async fn publish_due_scheduled_upload_releases(
    pool: &SqlitePool,
    creator_filter: Option<&str>,
...
async fn reconcile_stale_presence_sessions(state: SharedState) -> AppResult<()> {
    let cutoff = active_presence_cutoff();
    let now = Utc::now().to_rfc3339();
...
async fn reconcile_stale_creator_live_socket_sessions_for_read(
    pool: &SqlitePool,
    creator_filter: Option<&str>,
    user_filter: Option<&str>,
) -> AppResult<()> {
...
async fn reconcile_single_creator_live_socket_session(
    state: SharedState,
    creator_id: &str,
    socket_id: &str,
...
async fn authorize_media_request(
    state: &SharedState,
    headers: &HeaderMap,
...
async fn validate_playback_session_token_for_path(
    pool: &SqlitePool,
    playback_token: &str,
    relative_path: &str,
...
async fn creator_can_access_media_path(
    pool: &SqlitePool,
    creator_id: &str,
    relative_path: &str,
) -> AppResult<bool> {
...
fn playback_path_allowed_for_asset(asset: &MediaAsset, relative_path: &str) -> bool {
...
async fn fetch_uploads(pool: &SqlitePool, creator_id: &str) -> AppResult<Vec<Upload>> {
...
async fn fetch_upload_by_id(pool: &SqlitePool, creator_id: &str, id: &str) -> AppResult<Upload> {
...
async fn fetch_upload_jobs(pool: &SqlitePool, creator_id: &str) -> AppResult<Vec<UploadJob>> {
...
async fn fetch_upload_job_by_id(
    pool: &SqlitePool,
    creator_id: &str,
    id: &str,
) -> AppResult<UploadJob> {
...
async fn fetch_upload_job_by_id_raw(
    pool: &SqlitePool,
    creator_id: &str,
    id: &str,
) -> AppResult<UploadJob> {
...
async fn fetch_admin_media_jobs(
    pool: &SqlitePool,
    status_filter: Option<&str>,
    creator_filter: Option<&str>,
    limit: i64,
    ...
    let rows = match (status_filter, creator_filter) {
        ...
        (None, None) => {
            sqlx::query("SELECT id, creator_id FROM upload_jobs ORDER BY updated_at DESC LIMIT ?")
                .bind(limit)
                .fetch_all(pool)
                .await?
        }
...
async fn fetch_admin_media_job_record(
    pool: &SqlitePool,
    creator_id: &str,
    job_id: &str,
...
async fn fetch_upload_job_by_id_global(
    pool: &SqlitePool,
    job_id: &str,
) -> AppResult<AdminMediaJobRecord> {
...
async fn fetch_upload_job_creator_id(pool: &SqlitePool, job_id: &str) -> AppResult<String> {
...
async fn fetch_upload_ingest_session(
    pool: &SqlitePool,
    creator_id: &str,
    job_id: &str,
...
async fn fetch_upload_ingest_sessions(
    pool: &SqlitePool,
    creator_id: &str,
...
async fn fetch_creator_upload_operations_response(
    pool: &SqlitePool,
    creator_id: &str,
...
fn summarize_creator_upload_operations(
...
async fn fetch_analytics(pool: &SqlitePool, creator_id: &str) -> AppResult<Vec<AnalyticsPoint>> {
    let rows = sqlx::query(
...
async fn fetch_top_content(pool: &SqlitePool, creator_id: &str) -> AppResult<Vec<TopContent>> {
    let rows = sqlx::query(
...
async fn fetch_revenue_entries(
    pool: &SqlitePool,
    creator_id: &str,
...
fn summarize_creator_analytics(analytics: &[AnalyticsPoint]) -> CreatorAnalyticsSummary {
...
async fn fetch_notifications_rows(
    pool: &SqlitePool,
    creator_id: &str,
...
async fn fetch_notification_delivery_by_id(
    pool: &SqlitePool,
    delivery_id: &str,
...
async fn fetch_notification_delivery_by_id_raw(
    pool: &SqlitePool,
    delivery_id: &str,
) -> AppResult<NotificationDeliveryRecord> {
    let row = sqlx::query(
...
async fn enqueue_notification_event(
    pool: &SqlitePool,
    kind: &str,
    body: &str,
...
async fn reconcile_notification_deliveries_for_read(
    pool: &SqlitePool,
...
async fn reconcile_single_notification_delivery(
    state: SharedState,
    delivery_id: &str,
...
async fn claim_notification_delivery_attempt(
    pool: &SqlitePool,
    delivery_id: &str,
    attempted_at: &str,
) -> AppResult<bool> {
...
async fn deliver_inbox_notification(
    pool: &SqlitePool,
...
async fn mark_notification_delivery_failed(
    pool: &SqlitePool,
    delivery: &NotificationDeliveryRecord,
    attempted_at: &str,
    error: &AppError,
...
async fn fetch_live_notification_recipient_user_ids(
    pool: &SqlitePool,
    streamer_id: &str,
...
async fn fetch_chat_messages_for_viewer(
    pool: &SqlitePool,
    stream_id: &str,
...
async fn next_chat_message_sequence(pool: &SqlitePool, stream_id: &str) -> AppResult<i64> {
    let row = sqlx::query(
        r#"
...
async fn ensure_stream_exists(pool: &SqlitePool, stream_id: &str) -> AppResult<()> {
    let fresh_cutoff = stale_live_ingest_cutoff();
...
async fn effective_live_viewer_count(pool: &SqlitePool, stream_id: &str) -> AppResult<i64> {
...
async fn count_active_live_viewer_sessions(pool: &SqlitePool, stream_id: &str) -> AppResult<i64> {
    let row = sqlx::query(
        r#"
        SELECT COUNT(*) AS count
        FROM (
...
async fn count_all_active_live_viewer_sessions(pool: &SqlitePool) -> AppResult<i64> {
    let row = sqlx::query(
        r#"
        SELECT COUNT(*) AS count
        FROM (
...
async fn fetch_live_viewer_sample_users(
    pool: &SqlitePool,
    stream_id: &str,
    limit: i64,
) -> AppResult<Vec<String>> {
...
async fn register_live_viewer_session(
    pool: &SqlitePool,
    stream_id: &str,
...
async fn register_creator_live_socket_session(
    pool: &SqlitePool,
    creator_id: &str,
    user_id: &str,
    session_token: Option<&str>,
...
async fn touch_live_viewer_session(
    pool: &SqlitePool,
    stream_id: &str,
    session_token: &str,
...
async fn touch_creator_live_socket_session(
    pool: &SqlitePool,
    creator_id: &str,
    session_token: &str,
    lease_connected_at: &str,
...
async fn disconnect_live_viewer_session(
    pool: &SqlitePool,
    stream_id: &str,
    session_token: &str,
    lease_connected_at: &str,
...
async fn disconnect_creator_live_socket_session(
    pool: &SqlitePool,
    creator_id: &str,
    session_token: &str,
    lease_connected_at: &str,
...
async fn register_collaboration_socket_session(
    pool: &SqlitePool,
    session: &CollaborationSessionView,
    identity: &RequestIdentity,
    session_token: Option<&str>,
...
async fn touch_collaboration_socket_session(
    pool: &SqlitePool,
    session_id: &str,
    session_token: &str,
    lease_connected_at: &str,
...
async fn disconnect_collaboration_socket_session(
    pool: &SqlitePool,
    session_id: &str,
    session_token: &str,
    lease_connected_at: &str,
...
async fn count_active_collaboration_socket_sessions(
    pool: &SqlitePool,
    session_id: &str,
) -> AppResult<i64> {
    let row = sqlx::query(
...
async fn count_all_active_collaboration_socket_sessions(pool: &SqlitePool) -> AppResult<i64> {
    let row = sqlx::query(
...
async fn count_all_active_creator_live_socket_sessions(pool: &SqlitePool) -> AppResult<i64> {
    let row = sqlx::query(
...
async fn fetch_continue_watching_entry(
    pool: &SqlitePool,
    user_id: &str,
...
fn streamer_from_row(row: sqlx::sqlite::SqliteRow) -> Streamer {
    Streamer {
        id: row.get("id"),
        handle: row.get("handle"),
        display_name: row.get("display_name"),
...
fn live_stream_from_row(row: sqlx::sqlite::SqliteRow) -> LiveStream {
...
async fn check_database(pool: &SqlitePool) -> AppResult<bool> {
    let db_ok: i64 = sqlx::query("SELECT 1").fetch_one(pool).await?.get(0);
    Ok(db_ok == 1)
}

fn media_path_for_relative(state: &SharedState, relative_path: &str) -> std::path::PathBuf {
    state.media_root.join(relative_path)
}

async fn ensure_parent_dir(path: &std::path::Path) -> AppResult<()> {
...
fn sanitize_storage_key(input: &str) -> AppResult<String> {
    if input.is_empty() {
...
fn require_upload_token(headers: &HeaderMap) -> AppResult<String> {
    let value = headers
        .get("x-upload-token")
        .ok_or(AppError::Unauthorized)?
        .to_str()
...
fn require_ingest_token(headers: &HeaderMap) -> AppResult<String> {
    let value = headers
        .get("x-ingest-token")
        .ok_or(AppError::Unauthorized)?
        .to_str()
...
async fn validate_upload_ingest_token(
    pool: &SqlitePool,
    creator_id: &str,
    job_id: &str,
    upload_token: &str,
...
const MAX_MEDIA_PROCESSING_ATTEMPTS: i64 = 3;

struct UploadPlaybackTarget {
    creator_id: String,
    upload: Upload,
    asset: MediaAsset,
}

struct LivePlaybackTarget {
    creator_id: String,
    asset_id: String,
    title: String,
    poster_relative_path: Option<String>,
    playback_relative_path: String,
    asset: MediaAsset,
}

struct PlaybackSessionRecord {
    id: String,
...
struct PlaybackAccessDecision {
    access_scope: String,
}

async fn fetch_pending_media_jobs(pool: &SqlitePool) -> AppResult<Vec<(String, String)>> {
    let rows = sqlx::query(
...
async fn fetch_upload_playback_target(
    pool: &SqlitePool,
    upload_id: &str,
...
async fn fetch_live_stream_playback_target(
    pool: &SqlitePool,
    stream_id: &str,
...
async fn resolve_upload_playback_access(
    pool: &SqlitePool,
    identity: Option<&RequestIdentity>,
...
fn resolve_upload_access_terms(
...
async fn fetch_active_creator_membership(
    pool: &SqlitePool,
    user_id: &str,
    creator_id: &str,
...
async fn schedule_media_processing(state: SharedState, creator_id: String, job_id: String) {
...
async fn fail_media_job_for_lease(
    pool: &SqlitePool,
    creator_id: &str,
    job_id: &str,
    message: &str,
...
async fn requeue_media_job_for_processing(
    pool: &SqlitePool,
    creator_id: &str,
    job_id: &str,
) -> AppResult<()> {
...
async fn ensure_media_asset_shell(
    pool: &SqlitePool,
    creator_id: &str,
    job: &UploadJob,
...
async fn fetch_media_assets(pool: &SqlitePool, creator_id: &str) -> AppResult<Vec<MediaAsset>> {
    reconcile_stale_media_processing_jobs_for_read(pool, Some(creator_id), None).await?;
    let rows = sqlx::query(
        r#"
...
async fn fetch_media_asset_by_upload_job(
    pool: &SqlitePool,
    creator_id: &str,
    job_id: &str,
) -> AppResult<MediaAsset> {
...
async fn fetch_media_asset_by_upload_id(
    pool: &SqlitePool,
    creator_id: &str,
    upload_id: &str,
) -> AppResult<MediaAsset> {
...
async fn fetch_media_asset_by_id_any_creator(
    pool: &SqlitePool,
    asset_id: &str,
) -> AppResult<MediaAsset> {
...
async fn fetch_playback_session_by_id(
    pool: &SqlitePool,
    session_id: &str,
) -> AppResult<PlaybackSession> {
...
async fn fetch_admin_playback_sessions(
    pool: &SqlitePool,
    creator_filter: Option<&str>,
...
async fn fetch_admin_playback_session_record(
    pool: &SqlitePool,
    session_id: &str,
...
async fn validate_playback_session(
    pool: &SqlitePool,
    session_id: &str,
    playback_token: &str,
) -> AppResult<PlaybackSession> {
...
fn playback_session_from_record(session: &PlaybackSessionRecord) -> PlaybackSession {
    PlaybackSession {
        id: session.id.clone(),
...
async fn fetch_playback_session_record_by_id(
    pool: &SqlitePool,
    session_id: &str,
...
async fn validate_playback_session_record(
    pool: &SqlitePool,
    session_id: &str,
    playback_token: &str,
) -> AppResult<PlaybackSessionRecord> {
...
async fn validate_playback_session_record_for_path(
    pool: &SqlitePool,
    playback_token: &str,
    relative_path: &str,
) -> AppResult<PlaybackSessionRecord> {
...
async fn validate_existing_playback_session_access(
    pool: &SqlitePool,
...
async fn expire_playback_session_by_id(pool: &SqlitePool, session_id: &str) -> AppResult<()> {
    let now = Utc::now().to_rfc3339();
    sqlx::query(
...
async fn expire_playback_sessions_for_upload(pool: &SqlitePool, upload_id: &str) -> AppResult<()> {
    let now = Utc::now().to_rfc3339();
    sqlx::query(
...
async fn reconcile_playback_sessions_for_user(
    pool: &SqlitePool,
    user_id: &str,
    creator_id: Option<&str>,
...
async fn reconcile_playback_sessions_for_read(
    pool: &SqlitePool,
    creator_id: Option<&str>,
    content_id: Option<&str>,
...
async fn reconcile_single_playback_session(
    state: SharedState,
    session_id: &str,
...
async fn reconcile_invalid_playback_sessions(state: SharedState) -> AppResult<()> {
...
fn default_audio_track_id(tracks: &[PlaybackAudioTrack]) -> Option<String> {
    tracks
        .iter()
...
fn default_caption_track_id(tracks: &[PlaybackCaptionTrack]) -> Option<String> {
    tracks
        .iter()
        .find(|track| track.is_default)
        .map(|track| track.id.clone())
...
#[derive(Clone, Debug)]
struct StoredMediaPreviewTrack {
    id: String,
    label: String,
...
fn default_preview_track_id(tracks: &[PlaybackPreviewTrack]) -> Option<String> {
    tracks
        .iter()
        .find(|track| track.is_default)
        .map(|track| track.id.clone())
...
async fn fetch_user_subtitle_preference(
    pool: &SqlitePool,
    user_id: Option<&str>,
...
async fn fetch_user_audio_preferences(
    pool: &SqlitePool,
    user_id: Option<&str>,
...
async fn fetch_media_asset_variants(
    pool: &SqlitePool,
    asset_id: &str,
...
async fn fetch_media_processing_runs(
    pool: &SqlitePool,
    creator_id: &str,
    asset_id: &str,
...
async fn fetch_media_preview_track_rows(
    pool: &SqlitePool,
    asset_id: &str,
...
#[derive(Clone, Debug)]
struct GeneratedHlsSubtitleTrack {
    relative_path: String,
    language: String,
    name: String,
    is_default: bool,
...
#[derive(Clone, Debug)]
struct GeneratedHlsPackage {
    master_relative_path: String,
    variants: Vec<GeneratedHlsVariant>,
    audio_tracks: Vec<GeneratedHlsAudioTrack>,
    subtitle_tracks: Vec<GeneratedHlsSubtitleTrack>,
}
...
async fn probe_media(path: &FsPath) -> AppResult<ProbedMedia> {
...
fn validate_probed_media(job: &UploadJob, media: &ProbedMedia) -> AppResult<()> {
...
async fn verify_media_integrity(input_path: &FsPath, media: &ProbedMedia) -> AppResult<()> {
...
fn plan_hls_variants(media: &ProbedMedia) -> AppResult<Vec<HlsVariantPlan>> {
    let width = media
        .width
        .ok_or_else(|| AppError::BadRequest("video width could not be determined".to_string()))?;
    let height = media
...
async fn write_hls_master_manifest(
    output_path: &FsPath,
...
async fn validate_generated_hls_package(
    master_path: &FsPath,
...
async fn fetch_creator_series_title(
    pool: &SqlitePool,
    creator_id: &str,
    series_id: &str,
) -> AppResult<Option<String>> {
...
async fn ensure_creator_series_season(
    pool: &SqlitePool,
    creator_id: &str,
    series_id: &str,
    season_number: i64,
...
fn media_api_url(relative_path: &str) -> String {
    format!("/api/v1/media/{relative_path}")
}

fn sanitize_slug(input: &str) -> AppResult<String> {
...
fn media_content_type(relative_path: &str) -> &'static str {
...
fn validate_collaboration_role(role: &str) -> AppResult<()> {
    match role {
...
fn validate_profile_update(input: &UpdateProfileRequest) -> AppResult<()> {
...
fn validate_settings_update(input: &UpdateSettingsRequest) -> AppResult<()> {
...
fn validate_collaboration_participant_state(state: &str) -> AppResult<()> {
    match state {
...
fn validate_collaboration_chat_mode(chat_mode: &str) -> AppResult<()> {
    match chat_mode {
        "shared" | "host_only" => Ok(()),
        other => Err(AppError::BadRequest(format!(
...
fn validate_collaboration_recording_policy(recording_policy: &str) -> AppResult<()> {
...
fn validate_collaboration_participant_transition(
    current: &str,
    next: &str,
    host_action: bool,
) -> AppResult<()> {
...
fn validate_pending_collaboration_invite(invite: &CollaborationInvite) -> AppResult<()> {
...
fn validate_redeemable_collaboration_mirror_grant(
    grant: &CollaborationMirrorGrant,
    participant: &CollaborationParticipant,
    session: &CollaborationSession,
) -> AppResult<()> {
...
fn transition_creator_operational_status(
    current: &str,
...
fn monetized_access_policy(access_policy: &str) -> bool {
    matches!(
        access_policy,
...
fn parse_optional_future_timestamp(value: Option<&str>) -> AppResult<Option<String>> {
...
async fn ensure_creator_upload_ingest_enabled(
    pool: &SqlitePool,
    creator_id: &str,
) -> AppResult<()> {
    let profile = fetch_creator_profile(pool, creator_id).await?;
...
async fn ensure_creator_collaboration_enabled(
    pool: &SqlitePool,
    creator_id: &str,
) -> AppResult<()> {
    let profile = fetch_creator_profile(pool, creator_id).await?;
...
async fn ensure_creator_can_manage_subscription_tiers(
    pool: &SqlitePool,
    creator_id: &str,
) -> AppResult<()> {
    let profile = fetch_creator_profile(pool, creator_id).await?;
...
async fn validate_creator_access_tier(
    pool: &SqlitePool,
    creator_id: &str,
    access_policy: &str,
...
async fn ensure_creator_can_publish_paid_content(
    pool: &SqlitePool,
    creator_id: &str,
) -> AppResult<()> {
    let profile = fetch_creator_profile(pool, creator_id).await?;
...
async fn ensure_creator_can_accept_paid_transactions(
    pool: &SqlitePool,
    creator_id: &str,
) -> AppResult<()> {
    let profile = fetch_creator_profile(pool, creator_id).await?;
...
fn from_json<T: serde::de::DeserializeOwned>(value: String) -> AppResult<T> {
...
mod tests {
    ...
    async fn setup_test_state() -> AppResult<(SharedState, CreatorProfile)> {
    ...
    async fn insert_creator_auth_session(
        pool: &SqlitePool,
        creator: &CreatorProfile,
    ) -> AppResult<String> {
    ...
    async fn insert_user_auth_session(
        pool: &SqlitePool,
        user_id: &str,
        scopes: &[&str],
    ) -> AppResult<String> {
    ...
    async fn insert_ready_collaboration_broadcast(
        pool: &SqlitePool,
        creator: &CreatorProfile,
    ) -> AppResult<Broadcast> {
        let broadcast = Broadcast {
    ...
    async fn insert_shared_chat_collaboration_for_current_broadcast(
        pool: &SqlitePool,
    ...
    async fn insert_live_stream_for_creator(
        pool: &SqlitePool,
        creator: &CreatorProfile,
    ) -> AppResult<String> {
    ...
    async fn copy_sqlite_fixture(source_db: PathBuf, target_db: &Path) -> AppResult<()> {
    ...
    async fn reset_creator_live_state(
        pool: &SqlitePool,
        creator: &CreatorProfile,
    ) -> AppResult<()> {
        let now = Utc::now().to_rfc3339();
        ...
        .execute(pool)
        .await?;
        sqlx::query("DELETE FROM live_streams WHERE streamer_id = (SELECT id FROM streamers WHERE handle = ?)")
            .bind(&creator.handle)
            .execute(pool)
            .await?;
    ...
    async fn insert_ready_broadcast(
        pool: &SqlitePool,
        creator: &CreatorProfile,
    ) -> AppResult<Broadcast> {
        let broadcast = Broadcast {
    ...
    async fn creator_live_event_count(pool: &SqlitePool, broadcast_id: &str) -> AppResult<i64> {
        let row = sqlx::query(
    ...
    async fn creator_notification_delivery_count(
        pool: &SqlitePool,
        creator_id: &str,
        kind: &str,
        broadcast_id: &str,
    ...
    async fn insert_test_notification_delivery(
        pool: &SqlitePool,
    ...
    async fn live_ingest_event_count_for_session(
        pool: &SqlitePool,
        session_id: &str,
        event_type: &str,
    ) -> AppResult<i64> {
    ...
    async fn insert_playback_session_for_upload(
        pool: &SqlitePool,
        upload_id: &str,
        user_id: Option<&str>,
    ...
    async fn seed_content_purchase_for_user(
        pool: &SqlitePool,
        user_id: &str,
        creator_id: &str,
        upload_id: &str,
    ...
    async fn insert_active_collaboration_session(
        pool: &SqlitePool,
        host_creator: &CreatorProfile,
        guest_creator_id: &str,
        guest_user_id: &str,
    ...
    async fn insert_collaboration_participant(
        pool: &SqlitePool,
        session_id: &str,
        user_id: &str,
        creator_id: Option<&str>,
    ...
    async fn insert_test_user_with_creator_profile(
        pool: &SqlitePool,
        user_id: &str,
        handle: &str,
        display_name: &str,
    ...
    async fn insert_collaboration_socket_session(
        pool: &SqlitePool,
        session_id: &str,
        user_id: &str,
        creator_id: Option<&str>,
    ...
    async fn publish_test_collaboration_event(
        state: &SharedState,
        session_id: &str,
        participant_id: &str,
        actor_user_id: &str,
    ...
    async fn terminating_ingest_creates_creator_broadcast_end_notification() -> AppResult<()> {
        ...
        let notifications = fetch_notifications_rows(&state.pool, &creator.id).await?;
        let ended_broadcast =
            fetch_broadcast_by_id(&state.pool, &creator.id, &connected.session.broadcast_id)
                .await?;
    ...
    async fn redeeming_mirror_grant_materializes_guest_pickup_broadcast() -> AppResult<()> {
        ...
        let redeemed =
            redeem_collaboration_mirror_grant_internal(&state, &identity, &grant.id).await?;
        assert_eq!(redeemed.state, "active");
        assert!(redeemed.activated_at.is_some());

        let pickups =
            fetch_collaboration_mirror_pickups_for_participant(&state.pool, &participant.id)
                .await?;
    ...
    async fn redeeming_mirror_grant_propagates_host_viewers_to_guest_channel() -> AppResult<()> {
        ...
        let guest_stream =
            fetch_live_stream_by_id(&state.pool, &format!("lv-{}-live", guest_creator.handle))
                .await?;
        let guest_control =
            fetch_creator_live_control_response(&state.pool, &guest_creator.id).await?;
    ...
    async fn ending_session_publishes_invite_revoked_event() -> AppResult<()> {
        ...
        let ended =
            end_collaboration_session(State(state.clone()), headers, Path(session.id.clone()))
                .await?
                .0;
    ...
    async fn participant_collaboration_events_hide_other_invites_and_grants() -> AppResult<()> {
        ...
        .await?;
        let hidden_grant =
            issue_mirror_grant_for_participant(&state, &session, &hidden_participant, "usr-1")
                .await?;
    ...
    async fn ending_broadcast_with_stale_ingest_records_terminal_ingest_event() -> AppResult<()> {
        ...
        .execute(&state.pool)
        .await?;
        sqlx::query("UPDATE broadcasts SET status = 'ready', ended_at = NULL, duration_sec = NULL WHERE id = ?")
            .bind(&broadcast.id)
            .execute(&state.pool)
            .await?;
        sqlx::query("UPDATE creator_profiles SET live_status = 'ready', current_broadcast_id = ? WHERE id = ?")
            .bind(&broadcast.id)
            .bind(&creator.id)
            .execute(&state.pool)
            .await?;
    ...
    async fn playback_token_cannot_access_source_media_path() -> AppResult<()> {
        ...
        let poster =
            validate_playback_session_token_for_path(&state.pool, &playback_token, poster_path)
                .await?;
        ...
        let source_error = validate_playback_session_token_for_path(
            &state.pool,
            &playback_token,
            &asset.source_path,
        )
    ...
    async fn revoking_auth_session_expires_bound_playback_session() -> AppResult<()> {
        ...
        let error =
            validate_playback_session(&state.pool, &grant.session.id, &grant.playback_token)
                .await
    ...
    async fn admin_can_inspect_and_reconcile_playback_session_by_id() -> AppResult<()> {
        ...
        let report = reconcile_admin_playback_session(
            State(state.clone()),
            headers,
            Path(session_id.clone()),
        )
    ...
    async fn hls_master_manifest_contains_variant_entries() -> AppResult<()> {
        let temp_root =
            std::env::temp_dir().join(format!("lifestream-hls-test-{}", Uuid::new_v4()));
    ...
    async fn generated_hls_package_validation_rejects_missing_variant_playlist() -> AppResult<()> {
        let temp_root =
            std::env::temp_dir().join(format!("lifestream-hls-invalid-{}", Uuid::new_v4()));
    ...
    async fn generated_hls_package_validation_accepts_complete_artifact_graph() -> AppResult<()> {
        let temp_root =
            std::env::temp_dir().join(format!("lifestream-hls-valid-{}", Uuid::new_v4()));
    ...
    #[tokio::test]
    async fn user_can_inspect_and_reconcile_membership_and_purchase_entitlements_by_id()
    -> AppResult<()> {
        let (state, _creator) = setup_test_state().await?;
        ...
        .await?
        .0;
        let purchase_report = reconcile_my_purchase_entitlement(
            State(state.clone()),
            headers,
            Path(purchase_id.clone()),
        )
    ...
    #[tokio::test]
    async fn admin_playback_listing_filters_after_reconciling_invalid_newer_sessions()
    -> AppResult<()> {
        let (state, creator) = setup_test_state().await?;
    ...
    async fn collaboration_presence_counts_distinct_participants_not_socket_tabs() -> AppResult<()>
    {
        ...
        let per_session =
            count_active_collaboration_socket_sessions(&state.pool, &session.id).await?;
    ...
    -> AppResult<()> {
        ...
        let control =
            fetch_authoritative_creator_live_control_response(&state, &creator.id).await?;
        let runtime =
    ...
    async fn category_live_totals_follow_active_stream_truth_not_snapshot_columns() -> AppResult<()>
    {
        ...
        let gaming_stream_id =
            insert_live_stream_for_creator(&state.pool, &refreshed_creator).await?;
        let music_stream_id =
    ...
    async fn live_viewer_resume_token_cannot_be_rebound_to_another_user() -> AppResult<()> {
        ...
        let (session_token, resumed, _) =
            register_live_viewer_session(&state.pool, &stream_id, Some(&first_identity), None)
                .await?;
        assert!(!resumed);
    ...
    async fn stale_live_viewer_disconnect_cannot_tombstone_resumed_session() -> AppResult<()> {
        ...
        disconnect_live_viewer_session(&state.pool, &stream_id, &session_token, &first_lease)
            .await?;
    ...
    async fn stale_creator_live_disconnect_cannot_tombstone_resumed_session() -> AppResult<()> {
        ...
        disconnect_creator_live_socket_session(
            &state.pool,
            &creator.id,
            &session_token,
            &first_lease,
    ...
    async fn stale_collaboration_disconnect_cannot_tombstone_resumed_session() -> AppResult<()> {
        ...
        let session_view = fetch_current_collaboration_socket_session_view(
            &state,
        ...
        let (session_token, resumed, first_lease) =
            register_collaboration_socket_session(&state.pool, &session_view, &identity, None)
                .await?;
        assert!(!resumed);
        tokio::time::sleep(Duration::from_millis(5)).await;
    ...
    async fn shadowbanned_chat_history_is_visible_only_to_sender() -> AppResult<()> {
        ...
        let sender_history =
            fetch_chat_messages_for_viewer(&state.pool, &stream_id, Some("usr-2"), 50, None)
                .await?;
        let other_history =
            fetch_chat_messages_for_viewer(&state.pool, &stream_id, Some("usr-1"), 50, None)
                .await?;
        let sender_replay =
            fetch_chat_messages_for_viewer(&state.pool, &stream_id, Some("usr-2"), 50, Some(1))
                .await?;
    ...
    async fn collaboration_resume_bootstrap_does_not_duplicate_snapshot_events() -> AppResult<()> {
        ...
        let (resumed_snapshot, resumed_replay) =
            load_collaboration_socket_event_bootstrap(&state.pool, &session.id, first.sequence)
                .await?;
    ...
    async fn revoking_live_moderation_action_publishes_realtime_event() -> AppResult<()> {
        ...
        let event = tokio::time::timeout(Duration::from_secs(1), subscription.recv())
            ...
            .map_err(|_| {
                AppError::Internal(
                    "timed out waiting for moderation revoke realtime event".to_string(),
                )
            })?
            .map_err(|error| {
                AppError::Internal(format!(
    ...
    #[tokio::test]
    async fn expired_live_moderation_lookup_self_heals_stored_state_and_active_gate()
    -> AppResult<()> {
        let (state, creator) = setup_test_state().await?;
        let stream_id = insert_live_stream_for_creator(&state.pool, &creator).await?;
        let action_id = format!("test-lma-{}", Uuid::new_v4().simple());
    ...
    #[tokio::test]
    async fn runtime_dependencies_fail_closed_when_background_worker_never_succeeded()
    -> AppResult<()> {
        let (state, _) = setup_test_state().await?;
